...
def bn_mode_params(mode: str) -> tuple[int, int, int]:
    ...
    if mode == "160ms":
        return 4, 16, 19
    raise ValueError(f"Unsupported bn_frame_mode '{mode}'. Use '80ms', '160ms', or '200ms'.")


def callable_asr_cache(mode: str, device: torch.device) -> tuple[int, int, torch.Tensor, torch.Tensor]:
    if mode == "80ms":
        return (
            4,
            4,
            torch.zeros((6, 4, 4, 128), device=device),
            torch.zeros((6, 1, 256, 8), device=device),
        )
    if mode == "160ms":
        return (
            8,
            8,
            torch.zeros((6, 4, 8, 128), device=device),
            torch.zeros((6, 1, 256, 8), device=device),
        )
    raise ValueError(f"Callable ASR checkpoints are supported for '80ms' and '160ms', got '{mode}'.")
...
def extract_bn_from_audio(asr_model, wav: np.ndarray, sample_rate: int, mode: str, device: torch.device) -> np.ndarray:
    ...
    uses_forward_encoder_chunk = hasattr(asr_model, "forward_encoder_chunk")
    if uses_forward_encoder_chunk:
        ...
        offset, required_cache_size, att_cache, cnn_cache = callable_asr_cache(mode, device)
    bns = []
    ...
    with torch.no_grad():
        for i in range(0, max(fbanks.shape[1] - window + 1, 0), step):
            ...
            if uses_forward_encoder_chunk:
                ...
                encoder_output, att_cache, cnn_cache = asr_model(
                    fbank,
                    torch.tensor(offset, dtype=torch.int64, device=device),
                    torch.tensor(required_cache_size, dtype=torch.int64, device=device),
                    att_cache,
                    cnn_cache,
                )
